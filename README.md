...
GraphStack.io GDB Browser Browser is a fork of the Neo4j Browser.
...
`yarn e2e` to run the cypress js test suite (requires a **fresh** installation of neo4j to run against, expects neo4j 4.2 by default).
...
`yarn e2e-local-open --env server=4.2`  
...
Example: `yarn e2e-local-open --env server=4.2,include-import-tests=true`

Here are the available options / env variables:

```
server=3.2|3.3|3.4|3.5|4.0|4.1|4.2 (default 4.2)
...
yarn jar
yarn prepare-jar
mvn -o -Duser.name=graphstack install # Don't clean install.
